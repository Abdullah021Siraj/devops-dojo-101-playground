...
for pkg in "${DANGEROUS_PACKAGES[@]}"; do
    if dpkg -l 2>/dev/null | grep -q "^ii.*$pkg" \
       || rpm -qa 2>/dev/null | grep -q "$pkg"; then
        log_result "FAIL" "PCI 2.2.2" \
        "Insecure package installed: $pkg" \
        "Remove with: apt remove $pkg (or yum remove)"
        FOUND_DANGEROUS=1
    fi
done
...
# Check for running FTP service
if systemctl is-active --quiet vsftpd 2>/dev/null; then
    log_result "WARN" "PCI 2.2.2" \
    "FTP service (vsftpd) is running" \
    "Use SFTP instead of FTP"
    FOUND_DANGEROUS=1
fi

# PASS if nothing insecure found
if [ "$FOUND_DANGEROUS" -eq 0 ]; then
    log_result "PASS" "PCI 2.2.2" \
    "No insecure or legacy services detected" \
    "System compliant"
fi
